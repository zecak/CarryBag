...
namespace 随身袋.Helper
{
    public class NLCalendar
    {
        ...
        static string GetLunarCalendar(DateTime dtDay, ref string strYear, ref string strMonth, ref string strDate)
        {
            ...
            if (month < 1)
            {
                ...
                strMonth = "闰" + MonthName[-1 * month].ToString() + "月";
            }
            else
            {
                calendar += MonthName[month].ToString() + "月";
                strMonth = MonthName[month].ToString() + "月";
            }

            //农历日
            calendar += DayName[day].ToString() + "日";
            strDate = DayName[day].ToString();
            return calendar;

        }

        public static string GetCalendar(DateTime now)
        {
            var y = "";
            var m = "";
            var d = "";
            return GetLunarCalendar(now, ref y, ref m, ref d);
        }
    }
}
